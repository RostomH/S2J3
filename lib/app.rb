...
def tweet_a_journalist(list)
  journalists_sample = list.sample(5)
  journalists_sample.each do |i|
    login_twitter.update ("#{i} Merci pour le travail d'info et vive la France ! #bonjour_monde @the_hacking_pro")
  end
end

def like_tweets_with_a_given_hashtag
  login_twitter.search('#bonjour_monde').take(5).each do |tweet|
    login_twitter.favorite(tweet) 
  end
end

like_tweets_with_a_given_hashtag